...
<?php
		...
		for ( $i = 1; $i <= $blurbs_number; $i++ ){
			$foxy_home_page_number = et_get_option( 'foxy_home_page_' . $i );
			$foxy_home_page_number = ! empty( $foxy_home_page_number ) ? $foxy_home_page_number : 0;
			$service_query        = new WP_Query( apply_filters( 'et_service_query_args', 'page_id=' . get_pageId( $foxy_home_page_number ), $i ) );
